...
function pms_dc_apply_discount_success_message( $code, $subscription, $user_checked_auto_renew, $pwyw_price = '') {
    ...
    if ( !empty( $code ) && !empty( $subscription ) ) {
        ...
        // Set initial payment response
        if ( !empty( $response_trial ) && empty( $subscription_plan->sign_up_fee ) ) {
            $response_initial_payment = sprintf(__(' %s is %s', 'paid-member-subscriptions'), $response_trial, $initial_payment_price);
            $response_trial = '';
        }
        else if ( !$is_recurring && !empty( $response_trial) && !empty( $subscription_plan->sign_up_fee) ) {
            $after_trial_payment = ( function_exists('pms_format_price') && function_exists('pms_get_active_currency') ) ? pms_format_price((float)$subscription_plan->price, pms_get_active_currency()) : (float)$subscription_plan->price . $currency_symbol;

            $response_initial_payment = sprintf( __('now is %s, then %s %s', 'paid-member-subscriptions'), $initial_payment_price, $response_trial, $after_trial_payment );
        } else
            $response_initial_payment = sprintf( __(' is %s', 'paid-member-subscriptions'), $initial_payment_price );


        // Set recurring payment response
