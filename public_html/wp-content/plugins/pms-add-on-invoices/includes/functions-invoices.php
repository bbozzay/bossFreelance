    ...
    if( empty( $payment_id ) ) {
        ...
        if( empty( $settings['invoices']['company_details'] ) ) {
            ...
            if( empty( $invoice_number ) ) {
                ...
                if( empty( $payment->amount ) || $payment->status != 'completed' ) {
...
function pms_inv_generate_invoice_pdf_listener() {
    ...
    $pdf_invoice->SetTitle( apply_filters( 'pms_inv_invoice_title', ( ! empty( $settings['invoices']['title'] ) ? $settings['invoices']['title'] : __( 'Invoice', 'paid-member-subscriptions' ) ), $invoice_number, $payment->id ) );
